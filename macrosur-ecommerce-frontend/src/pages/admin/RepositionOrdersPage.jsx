...
import { FaBox, FaPlus, FaCheck, FaTimes, FaTruck } from 'react-icons/fa';
...
const OrdersPage = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      const [ordenesData, proveedoresData, variantesData] = await Promise.all([
        ...
        obtenerProveedores(),
        // Cambio: usar endpoint que filtra solo variantes que necesitan reposición
        axios.get('http://localhost:8081/api/productos/variantes/reposicion', { headers })
      ]);
  ...
  const handleCrearOrden = async () => {
    try {
      // Validar proveedorId
      if (!crearForm.proveedorId || crearForm.proveedorId === '') {
        alert('Por favor seleccione un proveedor');
        return;
      }

      // Validar items
      const itemsValidos = crearForm.items.filter(item => 
        item.varianteId && item.varianteId !== '' && 
        item.cantidadPedida && parseInt(item.cantidadPedida) > 0
      );
      ...
      setCrearForm({ proveedorId: '', items: [{ varianteId: '', cantidadPedida: '' }] });
      cargarDatos();
      ...
      <Modal show={showCrearModal} onHide={() => setShowCrearModal(false)} size="lg">
        ...
        <Modal.Body>
          <Form>
            ...
            <h5 className="mb-3 text-success">📦 Productos de la Orden</h5>
            {crearForm.items.map((item, index) => (
              <Row key={index} className="mb-3 border border-info p-2 rounded">
                <Col md={5}>
                  ...
                  >
                    ...
                    {[...variantes]
                      .sort((a, b) => {
                        // Ordenar por código de producto, luego por SKU
                        const codigoA = a.producto?.codigoProducto || '';
                        const codigoB = b.producto?.codigoProducto || '';
                        if (codigoA !== codigoB) {
                      ...
                      .map(v => {
                        // Ocultar SKU si es genérico (formato SKU-00XXX)
                        const isGenericSku = v.sku?.startsWith('SKU-');
                        const variantInfo = isGenericSku 
                          ? '' 
                          : ` - ${v.sku}`;
                        
                        // Mostrar información de stock
                        const stockInfo = v.stockActual !== undefined 
                          ? ` [Stock: ${v.stockActual}/${v.stockMinimo}]`
                          : '';
                        
                        return (
                          <option key={v.varianteId} value={v.varianteId}>
                            [{v.producto?.codigoProducto || 'N/A'}] {v.producto?.nombreProducto || 'Sin nombre'}{variantInfo}{stockInfo}
                          </option>
                        );
                      })
                    }
                  </Form.Select>
                  <small className="text-muted">Solo se muestran productos con stock bajo o sin inventario</small>
                </Col>
                <Col md={4}>
                  ...
                  />
                  {item.varianteId && (() => {
                    const variante = variantes.find(v => v.varianteId === parseInt(item.varianteId));
                    if (variante && variante.stockMinimo > 0) {
                      const sugerido = Math.max(variante.stockMinimo * 2 - variante.stockActual, 0);
                      return <small className="text-info">Sugerido: {sugerido} uds</small>;
                    }
                    return null;
                  })()}
                </Col>
